...
class anonymizer_loader:
    def __init__(self, cfg_path='./config/config.json', nmels=40):
        ...
        self.nmels = nmels
        self.setup_cuda()
        print(os.path.join(self.params['file_path'], "crema_d_metadata.csv"))
        self.main_df = pd.read_csv(os.path.join(self.params['file_path'], "crema_d_metadata.csv"), sep=';')
    ...
    def do_anonymize(self, mcadams_coef=0.8, output_utter_dirname='mcadams_anonymized'):
        self.speaker_list = self.main_df['speaker_id'].unique().tolist()
        for speaker_name in tqdm(self.speaker_list):
            ...
            for _ , row in selected_speaker_df.iterrows():
                original_path = os.path.join(self.file_path, row['relative_path'])

                utterance, sr = sf.read(original_path)

                output_path = os.path.join(self.file_path, row['relative_path'].replace('AudioWAV', output_utter_dirname))
